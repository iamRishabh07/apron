...
screen = pg.display.set_mode((WIDTH, HEIGHT + 100), 0, 32)  # 400x500 display
...
def draw_status():
    ...
    # Fix: position text inside the status bar (y=450), not at the center of the board
...
def drawXO(row, col):
    ...
    if current_player == 'x':
        pg.draw.line(screen, (0, 0, 0), (pos_y, pos_x),
                     (pos_y + size, pos_x + size), 5)
        pg.draw.line(screen, (0, 0, 0), (pos_y, pos_x + size),
                     (pos_y + size, pos_x), 5)
        current_player = 'o'
    else:
...
def reset_game():
    ...
    # Fix: pause for 10 seconds as required by the prompt
    time.sleep(10)
    current_player = 'x'
    is_draw = False
    # Fix: reset current_winner BEFORE calling game_initiating_window so
    # draw_status() inside it shows the correct new-game message
    current_winner = None
    grid = [[None]*3, [None]*3, [None]*3]
